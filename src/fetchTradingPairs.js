...
async function fetchAvailableTradingPairs() {
  ...
  async function filterCoinsByAvailability(coinLists) {
    ...
    return coinLists.map(coinList => coinList.filter(coin => availableTradingPairs.includes(coin)));
  }
  ...
  async function testFilterFunction() {
    ...
    const allCoinLists = [coinListA, coinListB, coinListC, coinListD, coinListE, coinListF];
